...
export const getConfigMap = (namespace: string, name: string) =>
  apiClient.get<ConfigMap>(`/config-map/${namespace}/${name}`).then((r) => r.data);
...
export const updateConfigMap = (namespace: string, name: string, payload: CreateConfigMapRequest) =>
  apiClient.put<MutationResponse>(`/config-map/${namespace}/${name}`, payload).then((r) => r.data);

export const deleteConfigMap = (namespace: string, name: string) =>
  apiClient.delete<MutationResponse>(`/config-map/${namespace}/${name}`).then((r) => r.data);
